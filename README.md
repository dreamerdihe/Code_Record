...
* **2. Add Two Numbers**: Just use recursion is enough

* **234. Palindrome Linked List**: For(space complexity O(1))First use two point to find the middle point, and then reverse the right half part of the list. We compare the left part with the right reversed list.

* **147. Insertion Sort List**: This problem is very good. First, initiate a dummy node for result. And every time we connect the already sorted part to dummy, and the rest part is the unsorted part. So, every time we find a suitable position for insertion. **Triky:** In the start of while loop, we check `prev.val > current.val`. If yes, we let the prev start from dummy, otherwise just start from last prev. **Remark:** remember the `current.next` before any operation, since after insertion, `current.next` has been changed.

* **160. Intersection of Two Linked Lists**: Calculate the lengthA and lengthB of the two linklists. And calculate the diffence `diff` of them. We let the longer one go `diff` times first, and then traverse to find the intersection.

* **24. Swap Nodes in Pairs**: Easy. But remember that you have to check whether there ramain one node when the number of nodes id odd. 

* **19. Remove Nth Node From End of List**: Two pointers, let the fast ponter go n steps first and then, let slow one go.

* **143. Reorder List**: Find the middle node by two pointer, reverse the right part of the linklist. Finally, connect them. **Note**: since left part could have one more element than right part, so you have to check whether the right node is null to make the program valid

* **445. Add Two Numbers II**: Use stack to store every digit.

* **203. Remove Linked List Elements**:  **Note**: when you delete a node, don't move `now` to the next node. 
